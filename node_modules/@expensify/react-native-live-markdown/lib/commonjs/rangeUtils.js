...
exports.groupRanges = groupRanges;
exports.sortRanges = sortRanges;
exports.splitRangesOnEmojis = splitRangesOnEmojis;
...
function splitRangesOnEmojis(ranges, type) {
  const emojiRanges = ranges.filter(range => range.type === 'emoji');
  const newRanges = [];
  let i = 0;
  let j = 0;
  while (i < ranges.length) {
    ...
    if (currentRange.type !== type) {
      ...
      while (j < emojiRanges.length) {
        ...
        const emojiStart = emojiRange.start;
        const emojiEnd = emojiRange.start + emojiRange.length;
